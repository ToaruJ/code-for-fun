...
def dpMuseumThief(treasureList, maxWeight):
    ...
    choosenList = []
    # 在这里补充你的代码
    dpvalue = [[0]*treasureList[0]['w']+[treasureList[0]['v']]*(maxWeight-treasureList[0]['w']+1)]                      #初始化记录动归数据的最大值和选取方法
    dpchoose = [[False]*treasureList[0]['w']+[True]*(maxWeight-treasureList[0]['w']+1)]                                 #取用该宝物为True，否则为False
    for i in range(1,len(treasureList)):
        dpvalue.append([])
        dpchoose.append([])
        for j in range(maxWeight+1):
            if j>=treasureList[i]['w'] and dpvalue[i-1][j-treasureList[i]['w']]+treasureList[i]['v']>dpvalue[i-1][j]:   #如果该宝物价值+剩余质量最大价值之和更大，则取
                dpvalue[i].append(dpvalue[i-1][j-treasureList[i]['w']]+treasureList[i]['v'])
                dpchoose[i].append(True)
            else:                                                                                                       #不取第i个宝物价值更大，或者取不动，则不取
                dpvalue[i].append(dpvalue[i-1][j])
                dpchoose[i].append(False)
    maxValue=dpvalue[len(treasureList)-1][maxWeight]
    weight=maxWeight
    for i in range(len(treasureList)-1,-1,-1):                                                                          #回推对第i个宝物是否选取
        if dpchoose[i][weight]:
            choosenList.append(treasureList[i])
            weight-=treasureList[i]['w']
    choosenList.reverse()
...
def dpWordEdit(original, target, oplist):
    ...
    operations = []
    # 在这里补充你的代码
    dpscore=[[i*oplist['delete'] for i in range(len(original)+1)]]                                                      #生成二维列表记录动归分数和操作
    dpop=[['delete']*(len(original)+1)]
    for i in range(1,len(target)+1):
        dpscore.append([i*oplist['insert']])
        dpop.append(['insert'])
        for j in range(1,len(original)+1):
            opr={dpscore[i][j-1]+oplist['delete']:'delete', dpscore[i-1][j]+oplist['insert']:'insert'}                  #用字典存下本次各个操作的分数
            if original[j-1]==target[i-1]:
                opr[dpscore[i-1][j-1]+oplist['copy']]='copy'
            dpscore[i].append(min(opr.keys()))                                                                          #将最小分数和操作方式记录到动归列表
            dpop[i].append(opr[min(opr.keys())])
    score=dpscore[len(target)][len(original)]
    row,col=len(target),len(original)                                                                                   #从动归列表顶点游走，得到总操作
    while row!=0 or col!=0:
        if dpop[row][col]=='copy':
            operations.append('copy '+original[col-1])
            row,col=row-1,col-1
        elif dpop[row][col]=='delete':
            operations.append('delete ' + original[col - 1])
            col-=1
        else:
            operations.append('insert ' + target[row - 1])
            row -= 1
    operations.reverse()
